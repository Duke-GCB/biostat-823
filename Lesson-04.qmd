...
CREATE TABLE Course (
  ...
  UNIQUE (Name, Semester) 
      -- table constraints must be at the end
...
;
```
```{sql, label=insertLesson03, echo=FALSE}
INSERT INTO Lesson (Name, Instructor_OID, Course_OID)
SELECT l.*, i.Instructor_OID, c.Course_OID
...
* Join order does not matter for inner joins, but does matter for whether a join must outer and left or right.
```{sql, label=selectJ5}
SELECT DISTINCT i.Name AS Instructor, c.Name AS Course, c.Semester, r.Name AS Room
...
## Restricting aggregate groups II {.smaller}
